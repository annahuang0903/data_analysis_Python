...
fh = open('newfile1.txt', 'w') # 'w' = 'write'
...
# If we add 'file=f' as an argument to the 'print' function
# then the output that would print to the console is instead
# written to the file. 

for aline in lines:
    if aline.count(' 2014 ') > 0:
        sub = aline.split('2014')[0]+'2014' # Note: No '\n'
...
#### Writing a pandas DatafFame to a file
...
df.to_csv('newfile3.txt', sep=',')  # Previous line overwritten
df.to_csv('newfile3.txt', sep=';', mode='a')  # Set to append to file
